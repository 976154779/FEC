...
bool AttachSymbol( uint8_t *_symbol, iClock_t _timeRest) {
	...
	if (restDataUnitLen == 0) {	// 表示上一个dataUnit已经用完了，需要重新获取一个dataUnit
		...
		restDataUnitLen += sizeof(dataUnit_t);			//刷新dataUnit的（包含头部）长度
	...
	do {
		...
		if (hasGetDataUnit == 0) {
			...
			if (RingBufferTimedGet(rbDataUnit, buff, SymbolTimeRest) == 1) {
				memcpy(restDataUnitLen, pBuff + 2, 2);
				restDataUnitLen += sizeof(dataUnit_t);		//获取dataUnit的（包含头部）总长度
				hasGetDataUnit = 1;		//表示目前获取到了一个dataUnit
			}
...
void DetachSymbol(uint8_t *_symbol) {
	...
	if (firstRunging == 1) {//本函数只运行一次
		...
		firstRunging = 0;//确保本函数只在最初被调用的时候，只运行一次
		...
		switch (switchState) {
			case 0:	//get head
					...
					if (temp >= 0) {
						...
						pSymbol = temp > 0 ? dataUnitHeadLen : NULL;//指针后移/表示一个symbol已经用完了
						//head over
						//refresh

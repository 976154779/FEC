...
bool AttachSymbol( uint8_t *_symbol, iClock_t _timeRest) {
	...
	static uint16_t restDataUnitLen =0;										//（实际上还没有装入symbol的）剩余的dataUnit长度	//必须保存为常量
	...
	if (restDataUnitLen == 0) {	//restDataUnitLen = 0; 表示上一个dataUnit已经用完了，需要重新获取一个dataUnit
		//先取第一个dataUnit
		RingBufferGet(rbDataUnit, buff);	//从rbDataUnit中获取dataUnit，放到pBuff所指向的地址里去
		memcpy(restDataUnitLen, pBuff + 2, 2);
		restDataUnitLen += sizeof(dataUnit_t);			//dataUnit的（包含头部）长度
		hasGetDataUnit = 1;		//表示目前获取到了一个dataUnit
	}
	else if (restDataUnitLen > 0) {//如果restDataUnitLen>0, 则上一次调用attachSymbol函数，生成了一个symbol后，一个dataUnit还没有用完，所以这次不需要再获取新的dataUnit
		hasGetDataUnit = 1;		//表示目前还是上一次没用完的dataUnit
	}

	do {
		TimeStart = iGetTime();						//获取当前时间

		if (hasGetDataUnit == 0) {
			//获取第二个（第三个……）dataUnit
			if (RingBufferTimedGet(rbDataUnit, buff, SymbolTimeRest) == 1) {
				memcpy(restDataUnitLen, pBuff + 2, 2);
				restDataUnitLen += sizeof(dataUnit_t);		//获取buff中所放入的dataUnit的（包含头部）长度
				hasGetDataUnit = 1;		//表示目前获取到了一个dataUnit
			}
		...
		if (restSymbolLen - restDataUnitLen > 0) {
			...
			restDataUnitLen = 0;
			pBuff = NULL;	//表示一个unit已经用完			
			pSymbol += restDataUnitLen;		//symbol指针后移
			pBuff = buff;	//指针重新指向buff
			...
			pBuff = restSymbolLen;	//表示一个unit已经用完			
			pSymbol = NULL;		//表示一个symbol已经生成完了
			restSymbolLen = SYMBOL_SIZE;
			SymbolTimeRest = SYMBOL_TIME;		//重新刷新构建Symbol的剩余的时间
			return 1;
		}
		else {
			//Unit over and symbol over
			memcpy(pSymbol, pBuff, restSymbolLen);//第三个参数或者填restDataUnitLen，都一样
			//refresh restSymbolLen and restDataUnitLen 
			restDataUnitLen = 0;
			restSymbolLen = SYMBOL_SIZE;
			pBuff = NULL;	//表示一个unit已经用完			
...
void GetBlock(uint8_t *_block, encInfo_t *_encInfo) {
	uint32_t symbolNumber = 0;//目前已经放入block中的symbol的个数
	iClock_t restBlockTime = BLOCK_TIME;//构建此block剩余的时间
	
	//_block = (void *)malloc(BLOCK_SYMBOL_NUMBER*SYMBOL_SIZE);//分配内存，用于存放Block
	uint8_t *pBlock = _block;//指针指向block内存开始的位置
	uint32_t symbolId = 0;//block当中symbol的序号，从0到31
	
	while (symbolNumber <= MAX_SYMBOL_NUMBER && restBlockTime > 0) {//最大不超过MAX_SYMBOL_NUMBER个Symbol构成一个Block, 此外每个block构建时间不超过BLOCK_TIME（10ms）
		...
		if (AttachSymbol(_symbol, restBlockTime) == 1) {//生成一个symbol
			iRaptorQPack(rbSend, _symbol, encInfo->_tag, symbolId, encInfo->_foreBSN);	//直接将源码打包后输出到rbSend待发送
			memcpy(pBlock, _symbol, SYMBOL_SIZE);
			pBlock += SYMBOL_SIZE;
			restBlockTime -= (iGetTime() - TimeStart);//刷新block剩余的时间
			symbolId++;
		}
		else {//生成一个symbol失败 //构建Block剩余的时间小于构建Symbol剩余的时间	
...
void DetachSymbol(uint8_t *_symbol) {
	...
	static uint8_t buff[MAX_UDP_DATA_SIZE + sizeof(dataUnit_t)];		//暂时保存将要输出的unit  //必须保存为常量
	static uint8_t *pBuff = buff;//指针指向buff的开始位置
	static uint16_t restSymbolLen = SYMBOL_SIZE;//剩余未处理symbol的长度
	...
	//状态机转换标志
	static bool switchState = 0;//0表示get head，1表示get data  //必须保存为常量

	static uint8_t firstRunging = 1;//首次运行本函数
	//get head
	if (firstRunging == 1) {//本函数只运行一次
		memcpy(pBuff, pSymbol, sizeof(dataUnit_t));//把dataUnit首部拷贝到buff中去
		memcpy(restDataLen, pSymbol + 2, 2);//将dataUnit中首部的len字段的值（代表着dataUnit中data的长度）拷贝到restDataLen
		pBuff += sizeof(dataUnit_t);//指针后移
		pSymbol += sizeof(dataUnit_t);//指针后移
		//head over
		//refresh
		...
		switch (switchState) {
			case 0:	
					...
					if (restSymbolLen - dataUnitHeadLen > 0) {
						...
						restDataUnitHeadLen = abs(restSymbolLen - dataUnitHeadLen);//保存下来，下一次继续获取剩下的dataUint头部
						memcpy(pBuff, pSymbol, restSymbolLen);//把dataUnit首部的部分拷贝到buff中去
			...
			case 1:	
					...
					if (restSymbolLen - restDataLen > 0) {
						memcpy(pBuff, pSymbol, restDataLen);//把dataUnit中的数据拷贝到buff中去
						pBuff += restDataLen;//指针后移
						pSymbol += restDataLen;//指针后移
						//data over
						//refresh
						...
						memcpy(pBuff, pSymbol, restDataLen);//把dataUnit中的数据拷贝到buff中去，第三个参数改为restSymbolLen也一样
						pBuff += restDataLen;//指针后移
						pSymbol = NULL;//表示一个symbol已经用完了
						//data over
						//new symbol
						...
						memcpy(pBuff, pSymbol, restSymbolLen);//把dataUnit中的数据(前半部分）拷贝到buff中去
						pBuff += restSymbolLen;//指针后移
						pSymbol = NULL;//表示一个symbol已经用完了
						//new symbol
						//refresh
